...
def render(duration=None, sr=None, t0=0.0, params=None):
    ...
    beat=60.0/bpm
    six=beat/4.0
    k=np.floor((t+float(t0))/six).astype(np.int64) % 32
    frac=((t+float(t0))/six) % 1.0
    env=_env_per_step(frac, atk=0.004, dec=28.0)

    hits = np.zeros(32, dtype=np.int32)
    for p in [0,6,12, 16,22]:
        hits[p]=1
    trig=hits[k].astype(np.float32)

    ph=2*np.pi*np.cumsum(np.full(n, 1400.0, dtype=np.float32))/sr
    body=np.sin(ph).astype(np.float32)*0.08
    m = (trig*1.0 + body*trig) * env * 0.65
    m=_hp1(m, 0.992)
    L,R=_pan(m, pan=-0.05)
