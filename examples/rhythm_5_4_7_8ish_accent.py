...
def render(duration=1.0, sr=48000, t0=0.0, params=None):
    ...
    for k in range(steps):
        ...
        pos = k % bar_steps
        amp = 0.0
        if pos in accents: amp = 0.25
        if pos in strong:  amp = max(amp, 0.55)
        if amp > 0.0:
            hit = _hat(sr, ms=10.0, g=amp)
            end = min(n, idx + hit.shape[0])
            if end > idx:
                out[idx:end] += hit[:end-idx]

    out = np.tanh(out * np.float32(1.15)).astype(np.float32)
    return _stereo(out)
