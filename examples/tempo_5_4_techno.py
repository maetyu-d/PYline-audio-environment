...
def render(duration=None, sr=None, t0=0.0, params=None):
    ...
    k16=np.floor((t+float(t0))/six).astype(np.int64)
    bar16 = 5*16
    pos = (k16 % bar16)

    frac = ((t+float(t0))/six) % 1.0
    env = _env_per_step(frac, atk=0.01, dec=14.0)

    kick_tr = ((pos==0)|(pos==32)|(pos==64)).astype(np.float32)
    clap_tr = (pos==48).astype(np.float32)
    hat_tr = ((pos % 8)==4).astype(np.float32)

    drop=np.exp(-t*20.0).astype(np.float32)
    f=65.0+170.0*drop
    ph=2*np.pi*np.cumsum(f)/sr
    kick=np.sin(ph).astype(np.float32) * np.exp(-t*9.0).astype(np.float32)
    kick = kick * kick_tr * (env*6.0)

    rng=np.random.default_rng(9)
    noise=rng.standard_normal(n).astype(np.float32)
    clap = _hp1(noise, 0.95)
    clap = _lp1(clap, 0.08) - _lp1(clap, 0.02)
    clap *= clap_tr * env * 0.35

    hats = _hp1(noise, 0.985) * hat_tr * env * 0.12
