...
def render(sr, duration, t0, params=None):
    ...
    spb = 60.0/bpm
    step = int(max(1, round(sr * (spb/8.0))))  # 32nds
    dens = float(params.get("density", 0.55))
    tau = float(params.get("tau_ms", 10.0))
    ...
    for i in range(0, n, step):
        if rng.random() < dens:
            y[i] += 1.0
            # occasional clustered extra ticks
            if rng.random() < 0.22 and i+step//3 < n:
                y[i+step//3] += 0.7
            if rng.random() < 0.12 and i+2*step//3 < n:
                y[i+2*step//3] += 0.5

    # give impulses a tiny tail
    klen = max(8, int(sr*0.004))
    k = _env_exp(klen, sr, tau_ms=tau)
