...
def render(duration=None, sr=None, t0=0.0, params=None):
    ...
    k=np.floor((t+float(t0))/trip).astype(np.int64)
    frac=((t+float(t0))/trip) % 1.0
    env=_env_per_step(frac, atk=0.006, dec=16.0)

    trig = ((k % 3) != 1).astype(np.float32)

    rng=np.random.default_rng(555)
    noise=rng.standard_normal(n).astype(np.float32)
    m=_hp1(noise, 0.985) * env * trig * 0.11
    L,R=_pan(m, pan=0.3)
