...
def render(duration=None, sr=None, t0=0.0, params=None):
    ...
    # 16th-note grid
    step = 60.0/bpm/4.0
    phase = (t + float(t0)) / step
    trig = (np.floor(phase) != np.floor(np.concatenate(([phase[0]-1], phase[:-1])))).astype(np.float32)

    # Make a "hard" click + slight pitchy body
    click = trig * 1.0
    body = np.sin(2*np.pi*(900 + 300*np.sin(2*np.pi*phase/16))*t) * trig*0.25

    env = _env_click(n, sr, tau_ms=6.0)
    m = (click + body) * env
    m = _hp1(m, 0.995) * 0.9
